...

{
  ...
  if [[ $MLB_STATUS -ne 0 || $NYT_STATUS -ne 0 ]]; then
    echo "WARNING: one or more scripts exited with errors (mlb=$MLB_STATUS nyt=$NYT_STATUS)"
  ...
  echo "--- git ---"
  GIT_STATUS=0
  if ! git add mlb-newspaper/index.html mlb-newspaper/nyt.html mlb-newspaper/mlb_cache.json; then
  ...
  # macOS notification
  if [[ $MLB_STATUS -eq 0 && $NYT_STATUS -eq 0 && $GIT_STATUS -eq 0 ]]; then
